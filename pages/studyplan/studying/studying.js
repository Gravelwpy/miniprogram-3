...
Page({
  ...
  onShow: function () {
    ...
    //要注意.这里取出来的是值的复制
    datas = wx.getStorageSync('key');    //取出缓存
  ...
  calculateRemain: function(list){           //计算剩余的时间
    list.map(function(value,index,array){     //如何获取？？？
  ...
  abandon:function(id){
    wx.showModal({
      ...
      success: function(res){
        if(res.confirm){
          var data = datas.splice(id,1);   //进行删除当前
          wx.setStorageSync('key', datas);   //重新存储时间
